...
const commands = [
  	...
  	new SlashCommandBuilder().setName('search').setDescription('Replies 10 results from searching Youtube.')
		.addStringOption(option =>
			option.setName('keyword')
				...
				.setRequired(true)),
	new SlashCommandBuilder().setName('now').setDescription('Replies with currently playing audio.'),
	...
	new SlashCommandBuilder().setName('queue').setDescription('Replies with the audio queue, limited to 10 per page.')
		.addIntegerOption(option => 
			option.setName('page')
				.setDescription('default: 1')),
	new SlashCommandBuilder().setName('shuffle').setDescription('Shuffles the existing queue.'),
